...
function RepoInput({ onIngestComplete, token }) {
  ...
  return (
    <div className=" flex flex-col items-center gap-5 max-w-xl mx-auto p-8">
      <div className="flex flex-col items-center gap-3 text-center">
        <h1 className="pixel-heading text-6xl text-white">
        ...
        <p className="text-slate-500 text-sm max-w-md font-mono">
          Chat with any repo—backed by real code, not hallucinations.
        </p>
      </div>
...
function RepoInput({ onIngestComplete, token, onBack }) {
  const [repoUrl, setRepoUrl] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  ...
  return (
    <div className="flex flex-col items-center gap-5 max-w-xl mx-auto p-8">
      <button
        onClick={onBack}
        className="self-start text-slate-400 text-xs font-mono hover:text-pink-300"
      >
        ← MY REPOS
      </button>
